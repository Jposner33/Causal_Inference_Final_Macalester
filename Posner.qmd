...
mod2 <- lm(ANA_win ~ Home * (Opp_Win_P_Cat + Opp_Playoffs + Opp_WS + Dow + Day_Night + Away_SP_WAR + Home_SP_WAR), data = match_data_full_ATT_ANA, weights = weights)
...
# make sure that it is 9.4 percent of 81 games and not 162
...
# Model Creation
mod3 <- lm(ANA_win ~ Home * (Opp_Win_P_Cat + Opp_Playoffs + Opp_WS + Dow + Day_Night + Away_SP_WAR + Home_SP_WAR), data = match_data_full_ATC_ANA, weights = weights)
...
# Model for ATE
mod4 <- lm(ANA_win ~ Home * (Opp_Win_P_Cat + Opp_Playoffs + Opp_WS + Dow + Day_Night + Away_SP_WAR + Home_SP_WAR), data = match_data_full_ATE_ANA, weights = weights)
...
```{r}
# Odds ratio based on ATE
evalues.OR(est = 0.0803, lo = 0.0433, hi = 0.117, rare = 0) # outcome of winning a game isn't rare, should be around 50% generally
```

> To explain away the observed causal effect, there would have to be an outstanding confounding variable that is 6.5 times more common in a team that wins than a team that loses. Winning would have to be 6.5 times more common in those that are affected by the confounding variable than those without it. This essentially says that our results are robust to unmeasured confounding variables. An unmeasured confounder would need to have an exceptionally strong relationship with home field and winning to explain away the observed effect on winning percentage. 
 
# Risk Difference
```{r}
# E-value based on exposed, diseased, non-exposed, non-diseased
n11 <- sum(ANA$Home==1 & ANA$ANA_win==1) # Home Game, Home Win
n10 <- sum(ANA$Home==1 & ANA$ANA_win==0) # Home Game, Home Loss
n01 <- sum(ANA$Home==0 & ANA$ANA_win==1) # Away Game, Away Win
n00 <- sum(ANA$Home==0 & ANA$ANA_win==0) # Away Game, Away Loss

evalues.RD(n11, n10, n01, n00, true = 0)
```

```{r}
CHN <- read.csv("Data/Team Datasets/CHN_games.csv")
...
# Matching Method for ATT
match_out_full_ATT_CHN <- matchit(
    Home ~ Opp_Win_P_Cat + Opp_Playoffs + Opp_WS + Dow + Day_Night + Away_SP_WAR + Home_SP_WAR,
    data = CHN,
    method = "full",
    exact = ~ Opp_Win_P_Cat + Opp_Playoffs + Opp_WS + Dow + Day_Night,  # Example of exact matching variables
...
```{r}
bal.tab(match_out_full_ATT_CHN)
```

```{r}
match_out_full_summ_ATT_CHN <- summary(match_out_full_ATT_CHN, interactions = TRUE)
plot(match_out_full_summ_ATT_CHN)
```
