...
# 'exact' was used to full match with additional precision variables. While no variables in this dataset directly effect the causal path of interest there are variables that may significantly impact game outcome. These variables were determined as the categorical win pct variable, if the opp made playoffs, if the opp made WS, day of the week, and night/day games. Why do these improve the model / matching?
```
...
> Change the interpretation for ATE - the estimate above is different than what is written below it. I think this occured because we didn't orignally have a model for the ATE, it was just based off of the ATT model from the very top. I changed this so that we have three distinct matching methods based on ATT, ATC, and ATE now. Nonetheless, the estimate is still very close to what we originally found and is statistically significant, with good support. 

> Organized the code and commented out some portions so that it is easier to follow. 

> Common support plots look good and confirm that the control and treated groups have matches that can be made when observing all cases. No changes need to be made in that regard. 

> Matching balance looks good, although we lose about 23 control cases. I am slightly confused as to why the standard mean difference for all variables and their interactions would be exactly zero. Why is that the case or is it incorrect? I could also be looking at the wrong thing on the output but I'm pretty sure that's what is displaying based on the graph. 
