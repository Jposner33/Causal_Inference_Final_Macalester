...
for (abbr in names(team_files)) {
  ...
  match_out_full <- matchit(
    Home ~ Opp_Win_P_Cat + Opp_Playoffs + Opp_WS + Dow + Day_Night + Away_SP_WAR,
    data = team_data,
    method = "full",
  ...
  formula_str <- paste0(dependent_var, " ~ Home * (Opp_Win_P_Cat + Opp_Playoffs + Opp_WS + Attendance + Dow + Day_Night)")
...
ATT_results<-ATT_results %>% 
  mutate(win_if_away = round(estimate * 81))

ggplot(ATT_results)+
  geom_col(aes(x = win_if_away, y = Team, fill = CI_Crosses_Zero))+
  labs(x = "Estimate Fewer Wins (If team played all 81 home games away)", y = "Team (Abv)", title = "Team Estimate Fewer Wins (ATT) by Team", fill = "Conf. Interval Crosses 0")+
...
```{r}
MLB_wins<-read.csv("Data/MLB_2024_Standings.csv") %>% 
  rename(Team = Team.Abbreviation)

MLB_wins_ATT<-ATT_results %>% 
  left_join(MLB_wins)

MLB_wins_ATT %>%
  mutate(
    ATT_wins = Total.Wins - win_if_away,
    Team = fct_reorder(Team, ATT_wins)
  ) %>% 
  filter(League == "American League") %>% 
  ggplot() +
  geom_col(aes(x = Total.Wins, y = Team, fill = "Total Wins"), alpha = 0.7) + 
  geom_col(aes(x = ATT_wins, y = Team, fill = "ATT Wins"), alpha = 0.7) +
  scale_fill_manual(
    name = "Win Type",
    values = c("ATT Wins" = viridis(2)[1], "Total Wins" = viridis(2)[2])
  ) +
  labs(
    title = "Comparison of Total Wins and Wins at Home",
    x = "Wins",
    y = "Team"
  ) +
  theme_minimal()
...
MLB_wins_ATT %>%
  ...
  labs(
    title = "Comparison of Total Wins vs. Wins if All Away in AL",
    x = "Wins",
    y = "Team"
  ) +
  theme_minimal()
