...
    Our results show that home field advantage has some effect on the standings in 2023. Miami, which was the 5th best team in the National League and made the playoffs, would've dropped to the 9th best team and missed the playoffs, while another team would've made the playoffs in Miami's place. Cincinnati also moved up a couple of spots and made the playoffs in this scenario. After seeing these results, we can see that while home field advantage affect a single game outcome, it can also affect whether or not a team makes the playoffs. 
...
    Miami has the largest estimate at about .13, which means that for all games in our analysis, if Miami played every away game as a home game, its winning percentage would increase by about .13, or it would win 13% more games. This contextually makes sense because Miami plays at a very low altitude, which decreases the flight of the ball making it a very pitcher friendly ballpark. Miami can use this to its advantage by signing different types of hitters or by spending more money on good pitchers than hitters. Miami is used to its ballpark conditions, and it could have an impact on its winning percentage at home. Texas and San Francisco also have estimates over .1, which indicates that both teams also have significant home field advantage over other teams. 
...
    The Cincinnati Reds have the lowest positive estimate where the confidence interval doesn't cross 0 at about .04. This means for all games in our analysis, if Cincinnati played every away game as a home game, its winning percentage would increase by about .04, or it would win about 4% more games. This means that there is no real change between a home game and an away game for Cincinnati, thus it doesn't have a large home field advantage. Boston and the New York Yankees also have low estimates, however, both teams' confidence intervals cross 0, which means their estimate isn't significant.

    Similar to our ATT analysis, the Cubs ATC estimate is also an interesting case and is the only negative estimate in our analysis. Its estimate is about -.1. We can interpret it as if the Cubs played every away game at home, meaning all 162 games were played at home, the Cubs would win about 10% fewer games. This is very significant, suggesting that the Cubs don't have a significant home field advantage. 
...
    Our results indicate that for the ATC, home field advantage has somewhat of an effect on the 2023 standings. In this scenario, teams that were on the bubble of making the playoffs, such as Miami or San Diego, would have made the playoffs in this situation. As mentioned above, the Cubs had a negative estimate and dropped significantly in these standings as a result of our analysis. The Cubs were on the cusp of a playoff berth and would've finished second to last as a result of our analysis. These results have a larger difference in standings when analyzing the ATT, and show that playing every game at home can have more of an effect on the 2023 standings. 

### Sensitivity Analysis

    To complete our sensitivity analysis, we ran a for loop on every team to calculate how many home wins, away wins, home losses, and away losses. Below is are plots of teams with the six largest and smallest estimated e-value.
...
    Colorado has the highest e-value at around 2.1 which means that the degree to which our analysis could be affected by a confounding variable is lower for them. This makes sense as well because the park is known to provide an advantage to hitters due to elevation and ball flight. Boston on the other hand has the lowest e-value at 1.44 which may suggest that our estimation and analysis for them is more sensitive to confounding variables. For Boston specifically, their park layout with the green monster may confound in addition to other variables. The mean e-value for every team in the data set is 1.6 which could be better and may suggest that there are uncounted variables that influence the relationship between winning and playing at home.

## Conclusions

    Looking at our results, playing at home does seem to provide a distinct advantage for several MLB teams, as evidenced by a significant increase in win percentage for about half of the league. Notably, teams with historically distinct home fields, such as Colorado (COL), Toronto (TOR), and New York Yankees (NYA), show a higher win percentage, suggesting that home field advantage may be influenced by factors like unique stadium features. However, certain teams, like the Reds and Cubs, did not demonstrate statistically significant improvements, possibly due to difficulty in matching or other external factors. The results may also be sensitive to confounding variables such as weather conditions, umpire crew, and stadium layout, though the average effect size (E-Value) suggests the impact of these confounders is relatively small. Overall, while there is some clear evidence of home-field advantage, further investigation is needed to refine the analysis and control for additional variables that could influence outcomes.
...
    All code and data to reproduce these visuals and analysis can be found in the `Analysis.qmd`, `data.qmd`, and  `SensAnalysis.qmd` files linked in our GitHub Repo. 